...
class ISRSet(utils.ImgSet):
    ...
    def __getitem__(self, index):
        ...
        center = l[np.abs(self.OPDs) <= utils.EPSILON][0]
        return (
...
class IHRFramework(utils.Framework):
    ...
    def get_model(self):
        ...
        func_A = Phi
        func_A_inv = Phi_inv
        func_mu = models.func_mu_1
        if type == "ISRUT":
            # func_mu = models.func_mu
            pass
        elif type == "IHRUT":
            return models.IHRUT(
            ...
            func_A = FFT
            func_A_inv = FFT_inv
            func_mu = models.func_mu_0
        elif type == "INV":
            func_mu = models.func_mu_0
        else:
            func_A_inv = Phi_inv0
            func_mu = models.func_mu_0
        return models.DeepUnfolding(
            len(self.intf_template.channels),
            # len(utils.LASISIntfImage.OPDs2bands(self.intf_template.channels)[0]),
            len(self.spec_template.channels),
    ...
    def forward(self, data: utils.Batch):
        data.src -= data.D
        model_out = self.model(data.src, data)
...
def Phi(tensor, data: utils.Batch):
...
def Phi_inv(tensor, data: utils.Batch):
    tensor /= data.M
    # utils.show_line(
    #     np.arange(256),
    #     tensor.cpu().detach().numpy()[..., 50, 12],
    #     "back_checkk1",
    # )
    ...
    #     np.arange(221),
    #     [
    #         tensor.real.cpu().detach().numpy()[..., 46, 12],
    #         tensor.imag.cpu().detach().numpy()[..., 46, 12],
    #     ],
    #     "back_checkk3",
    # )
    tensor = tensor / data.A
    ...
    #     np.arange(221),
    #     [
    #         data.A.real.cpu().detach().numpy()[..., 12],
    #         data.A.imag.cpu().detach().numpy()[..., 12],
    #     ],
    #     "back_checkk_A",
    # )
    # utils.show_line(
...
def FFT(tensor, data: utils.Batch):
    tensor = utils.HSImage.tensor_spec2bands_upsample(
        tensor, data.specs[0], data.bands[0]
    )
    tensor = utils.LASISIntfImage.tensor_bands2intf_fft(tensor, data.center)
...
def FFT_inv(tensor, data: utils.Batch):
    tensor = utils.LASISIntfImage.tensor_intf2bands_fft(tensor, data.center)
    # tensor /= torch.mean(data.A, dim=-1, keepdim=True)
    tensor = torch.abs(tensor)
    tensor = utils.HSImage.tensor_band2spec_sample(tensor, data.bands[0], data.specs[0])
    return tensor
