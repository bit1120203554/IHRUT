...
# 这个很好！是fuxian checkpoint的东西
# 记得参数mu个数是3的时候，stage interaction要有
# 记得初始mu必须是负的（也就是-0.5开始，或者yb-y开始）
...
# class ScanningAttention(nn.Module):


class ScanningAttention(nn.Module):

    def __init__(self, num_feat, squeeze_factor=8, memory_blocks=128):
        super(ScanningAttention, self).__init__()
    ...
    def forward(self, x):
        ...
        x = x * y2
        x = x.reshape(B, W, C, H).permute(0, 3, 1, 2)
        return x
...
class TransBlock(nn.Module):
    def __init__(
        ...
        self.sc_attn = ScanningAttention(dim)
...
class IHRTrans(nn.Module):  # no drop path, u-shaped
    def __init__(
        ...
        for i in range(2 * self.depth - 1):
            ...
            opt.dim = dim
            block = duf_mixs2_intf.MixS2_Block(
                opt, channels[i + 1], num_heads=num_heads[i]
            )
            self.body.append(
...
class DataBlock(nn.Module):
    def __init__(self, y_channels, out_channels):
        ...
        self.sc_attn = ScanningAttention(out_channels)

    # CA Module
    def forward(self, x_i, y, func_A, func_A_inv, mu, appendix):
        # compute r_k
        yb = func_A(x_i, appendix)
        r = y - yb
        r = func_A_inv(r, appendix)
        r = mu * self.sc_attn(rearrange(r, "b c h w -> b h w c"))
        r_i = x_i - rearrange(r, "b h w c -> b c h w")
        # r_i = x_i - mu * func_A_inv((yb - y), appendix)
        return r_i
...
class IHRUT(nn.Module):
    def __init__(
        ...
        self.mu = nn.Parameter(torch.full((self.num_stages,), 0.5))

        self.data_block = nn.ModuleList(
            [DataBlock(in_channels, out_channels) for _ in range(self.num_blocks)]
        )
    ...
    def forward(self, y, appendix):
        ...
        for i in range(self.num_stages):
            ...
            x = self.data_block[j](
                x, y, self.func_A, self.func_A_inv, self.mu[i], appendix
            )
            ...
            x, en, btn, dec = self.prior_block[j](x, en, btn, dec)
            x = x[..., :H, :W]
            j += 1
            pass
        return x
