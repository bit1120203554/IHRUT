...
class StripeAttention(nn.Module):
    ...
    def __init__(
        self, num_feat, num_heads=1, init_mu=0.5, squeeze_factor=8, memory_blocks=128
    ):
        super(StripeAttention, self).__init__()
        # 初始化mu参数，用于控制注意力权重
        self.num_heads = num_heads
        # self.mu = nn.Parameter(torch.full((num_heads, 1, 1, 1), init_mu))
        self.pool = nn.AdaptiveAvgPool1d(1)
        self.subnet = nn.Sequential(
            nn.Linear(num_feat, num_feat // squeeze_factor),
            # nn.ReLU(inplace=True)
        )
        self.mu_pool = nn.AdaptiveAvgPool1d(1)
        self.munet = nn.Sequential(
            nn.Linear(num_feat // squeeze_factor, num_feat // squeeze_factor),
            nn.ReLU(inplace=True),
            nn.Linear(num_feat // squeeze_factor, num_heads),
            nn.Softplus(),
        )
    ...
    def forward(self, x):
        ...
        low_rank_f = self.subnet(y)  # B*W, C//4 #, 1

        mu = self.mu_pool(low_rank_f.transpose(0, 1)).squeeze()
        mu = self.munet(mu).reshape(self.num_heads, 1, 1, 1, 1)
        mbg = self.mb.unsqueeze(0).repeat(B * W, 1, 1)  # B*W, C//4, 128
        f1 = (low_rank_f.unsqueeze(2).transpose(1, 2)) @ mbg
        f_dic_c = F.softmax(
        ...
        x = x * y2
        x = x.reshape(B, W, C, H).permute(0, 2, 3, 1) * mu.reshape(-1, 1, 1, 1, 1)
        return x  # 5D
...
class IHRTrans(nn.Module):
    def __init__(
        ...
        self.Encoder = nn.ModuleList(
            [
                TransBlock(dim=dim * 2**0, num_heads=2**0),
                TransBlock(dim=dim * 2**1, num_heads=2**1),
            ]
        )

        self.BottleNeck = TransBlock(dim=dim * 2**2, num_heads=2**2)
        ...
        # if self.block_interaction:
        #     self.BlockInteractions = nn.ModuleList(
        #         [
        #             BlockInteraction(dim * 7, dim * 1),
        #             BlockInteraction(dim * 7, dim * 2),
        #         ]
        #     )

        self.Downs = nn.ModuleList(
            [
                Sample(dim * 2**0, dim * 2**1, "down"),
                Sample(dim * 2**1, dim * 2**2, "down"),
            ]
        )

        self.Ups = nn.ModuleList(
            [
                Sample(dim * 2**2, dim * 2**1, "up"),
                Sample(dim * 2**1, dim * 2**0, "up"),
            ]
        )
        ...
        #         [
        #             StageInteraction(dim=dim * 2**0, act_fn_name=opt.act_fn_name),
        #             StageInteraction(dim=dim * 2**1, act_fn_name=opt.act_fn_name),
        #             StageInteraction(dim=dim * 2**2, act_fn_name=opt.act_fn_name),
        #             StageInteraction(dim=dim * 2**1, act_fn_name=opt.act_fn_name),
        #             StageInteraction(dim=dim * 2**0, act_fn_name=opt.act_fn_name),
        #         ]
        #     )
    ...
    def forward(self, x, enc_outputs=None, bottleneck_out=None, dec_outputs=None):
        ...
        # if self.stage_interaction and bottleneck_out is not None:
...
class DataBlock(nn.Module):
    def __init__(self, y_channels, out_channels, init_mu):
        ...
        # self.alp = nn.Parameter(torch.tensor(1.0))
    ...
    def forward(self, x_i, y, func_A, func_A_inv, appendix, alpha, k_last):
        ...
        kai = func_A_inv(y - yb, appendix)
        r_i = x_i - kai * alpha + self.sc_attn(x_i - k_last)[0]
        # r_i = x_i + self.sc_attn(-kai * alpha + x_i - k_last)
        # r_i = x_i - self.sc_attn(kai * alpha + k_last - x_i)  # 1

        # kai = func_A_inv(alpha * (y - yb), appendix)
        # r_i = x_i - self.sc_attn(kai + k_last - x_i) # 与1基本相同

        # kai = func_A_inv(alpha * (y - yb), appendix)
        # r_i = x_i - kai + self.sc_attn(k_last - x_i)  # 与1基本相同

        # r_i = x_i - self.sc_attn(kai * self.alp + k_last - x_i)
        return r_i
...
class IHRUT(nn.Module):
    def __init__(
        ...
        # self.para_estimator = HyPaNet(in_nc=in_channels, out_nc=num_stages)
        self.para_estimator = StripeAttention(
            num_feat=out_channels, num_heads=num_stages, init_mu=0.5
        )
        self.data_block = nn.ModuleList(
            [
                DataBlock(out_channels, out_channels, init_mu)
        ...
        self.prior_block = nn.ModuleList(
            [
                MAUN_intf.CSFormer(out_channels, out_channels, n_feat)
                for _ in range(self.num_blocks)
            ]
        )
    ...
    def forward(self, y, appendix):
        ...
        for i in range(self.num_stages):
            ...
            cross_last = cross_cur
            cross_cur = x
            hb, wb = (32, 32)
            H_pad = (hb - H % hb) % hb
            W_pad = (wb - W % wb) % wb
            x = F.pad(x, (0, W_pad, 0, H_pad), mode="reflect")
            cross_last = F.pad(cross_last, (0, W_pad, 0, H_pad), mode="reflect")
            x = self.prior_block[j](x, cross_last)  # [0]
            x = x[..., :H, :W]
