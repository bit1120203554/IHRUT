...
class BNReLUDeConv3d(nn.Sequential):
    def __init__(self, in_channels, channels, k=3, s=1, p=1, inplace=False):
        ...
        self.add_module("relu", nn.ReLU(inplace=inplace))
        self.add_module(
            "deconv", nn.ConvTranspose3d(in_channels, channels, k, s, p, bias=False)
        )
...
class BNReLUUpsampleConv3d(nn.Sequential):
    def __init__(
        ...
        self.add_module(
            "upsample_conv",
            UpsampleConv3d(
                in_channels, channels, k, s, p, bias=False, upsample=upsample
            ),
        )
    ...
    def __init__(
        ...
        if upsample:
            self.upsample_layer = torch.nn.Upsample(
                scale_factor=upsample, mode="trilinear", align_corners=True
            )
...
class BasicDeConv3d(nn.Sequential):
    def __init__(self, in_channels, channels, k=3, s=1, p=1, bias=False, bn=True):
        ...
        if bn:
            self.add_module("bn", BatchNorm3d(in_channels))
        self.add_module(
            "deconv", nn.ConvTranspose3d(in_channels, channels, k, s, p, bias=bias)
        )
