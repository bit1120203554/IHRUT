...
class SSMA(nn.Module):
    ...
    def calculate_mask(self, x_size, device="cpu"):
    ...
    def forward(self, x):
        ...
        if self.input_resolution == [H, W]:  # non-local speatial attention
            ...
            attn_windows = self.attn(
                x_windows,
                mask=self.calculate_mask([H, W], device=x.device),  # .to(x.device)
            )
        ...
        # t = [eval(f"t{i}") for i in range(8)]
        # print([t[i + 1] - t[i] for i in range(7)])
        return x
...
class SST(nn.Module):
    ...
    def __init__(
        ...
        self.conv_first = nn.Conv2d(
            inp_channels, dim, 3, 1, 1
        )  # shallow featrure extraction
        ...
        self.output = nn.Conv2d(
            int(dim), dim, kernel_size=3, stride=1, padding=1, bias=bias
        )
        self.conv_delasta = nn.Conv2d(
            dim, inp_channels, 3, 1, 1
