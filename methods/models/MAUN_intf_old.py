...
class CS_MSA(nn.Module):
    ...
    def forward(self, x, px):
        ...
        px_inp = rearrange(
            px, "b (h b0) (w b1) c -> (b h w) (b0 b1) c", b0=w_size[0], b1=w_size[1]
        )
...
class CSFormer(nn.Module):
    def __init__(self, in_dim=28, out_dim=28, dim=28):
        ...
        self.CS_MSA = CSAB(
            dim=dim_scale, num_blocks=1, dim_head=dim, heads=dim_scale // dim
        )
        self.CS_MSA2 = CSAB(
            dim=dim_scale, num_blocks=1, dim_head=dim, heads=dim_scale // dim
        )
        ...
        self.up1 = nn.ConvTranspose2d(
            dim_scale, dim_scale, stride=2, kernel_size=2, padding=0, output_padding=0
        )
...
class MAUN(nn.Module):
    ...
    def __init__(
        self, in_channels, out_channels, func_0, func_A, func_A_inv, num_iterations=7
    ):
        ...
        self.para_estimator = HyPaNet(in_nc=in_channels, out_nc=num_iterations)
        self.CAs = nn.ModuleList([])
        for _ in range(num_iterations):
            self.denoisers.append(
                CSFormer(in_dim=out_channels, out_dim=out_channels, dim=n_feat),
            )
            self.CAs.append(
                CA(out_channels),
            )
    ...
    def initial(self, y, appendix):
        ...
        #     ]
        z = self.func_A_inv(y, appendix)
        # z = self.fution(torch.cat([y_shift, appendix], dim=1))
        # alpha = self.para_estimator(self.fution(torch.cat([y_shift, appendix], dim=1)))
        alpha = self.para_estimator(y)
        return z, alpha

    def forward(self, y, appendix):
        # y = y.squeeze(dim=0)
        b, l, h_inp, w_inp = y.shape
        x, alphas = self.initial(y, appendix)
        ...
        for i in range(self.num_iterations):
            alpha = alphas[:, i, :, :]
            zb = self.func_A(sp, appendix)
            sp = sp + self.func_A_inv(torch.div(y - zb, alpha), appendix)
            # sp = shift_back_3d(sp)
        ...
        return p_cur[:, :, :h_inp, :w_inp]
        # return xl[:, :, :h_inp, :w_inp]
