...
class StripeAttention(nn.Module):
    ...
    def __init__(self, num_feat, out_feat=0, squeeze_factor=8, memory_blocks=128):
        ...
        self.outnet = nn.Conv2d(num_feat, out_feat, 1, 1, 0)
...
class TransBlock(nn.Module):
    def __init__(
        ...
        self.sc_attn = StripeAttention(dim, memory_blocks=dim // 2)
...
class DataBlock(nn.Module):
    def __init__(self, y_channels, out_channels):
    ...
    def forward(self, x_i, y, func_A, func_A_inv, appendix, alpha, k_last):
        ...
        # r_i = x_i - self.sc_attn(kai * alpha + k_last - x_i)[0]
        r_i = x_i + self.sc_attn(torch.div(kai, alpha) + x_i - k_last)  # 1
...
class IHRUT(nn.Module):
    def __init__(
        ...
        self.para_estimator = StripeAttention(
            num_feat=out_channels, out_feat=num_stages
        )
        self.data_block = nn.ModuleList(
            [DataBlock(out_channels, out_channels) for _ in range(self.num_blocks)]
        )
