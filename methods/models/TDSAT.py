...
class SpectralEnhancement(nn.Module):
    def __init__(self, ch):
        ...
        self.se = nn.Sequential(
            GlobalAvgPool3d(), nn.Conv3d(ch, ch, kernel_size=1), nn.Sigmoid()
        )
...
class MHSA3D(nn.Module):
    def __init__(self, channels=16, num_heads=1):
        ...
        self.project_out = nn.Conv3d(
            channels, channels, kernel_size=(1, 1, 1), bias=False
        )
...
class GDFN3D(nn.Module):
    def __init__(self, channels, expansion_factor):
        ...
        self.project_in = nn.Conv3d(
            channels, hidden_channels * 2, kernel_size=(1, 1, 1), bias=False
        )
        ...
        self.project_out = nn.Conv3d(
            hidden_channels, channels, kernel_size=(1, 1, 1), bias=False
        )
...
class SATB(nn.Module):
    ...
    def forward(self, x):
        b, t, c, h, w = x.shape
        x = x + self.attn(
            self.norm1(x.reshape(b, t, -1).transpose(-2, -1).contiguous())
            .transpose(-2, -1)
            .contiguous()
            .reshape(b, t, c, h, w)
        )
        x = x + self.ffn(
            self.norm2(x.reshape(b, t, -1).transpose(-2, -1).contiguous())
            .transpose(-2, -1)
            .contiguous()
            .reshape(b, t, c, h, w)
        )
...
class EmbeddingBlock(nn.Module):
    ...
    def forward(self, x):
        ...
        x = x + self.gcfn(
            self.norm3(x.reshape(b, t, -1).transpose(-2, -1).contiguous())
            .transpose(-2, -1)
            .contiguous()
            .reshape(b, t, c, h, w)
        )
                    ...
                    OrderedDict(
                        [
                            (
                                "conv",
                                nn.Conv3d(
                                    channels, channels * 2, 3, (1, 2, 2), 1, bias=False
                                ),
                            ),
                    ...
                    OrderedDict(
                        [
                            (
                                "conv",
                                nn.ConvTranspose3d(
                                    channels, channels, 3, 1, 1, bias=False
                                ),
                            ),
                            ("attn", Attn(channels)) if Attn else None,
                        ]
                ...
                decoder_layer = nn.Sequential(
                    OrderedDict(
                        [
                            ...
                            (
                                "conv",
                                nn.Conv3d(channels, channels // 2, 3, 1, 1, bias=False),
                            ),
                            ("attn", Attn(channels // 2)) if Attn else None,
                        ]
                    )
