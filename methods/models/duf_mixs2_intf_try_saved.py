...
def merge_duf_mixs2_opt(parser):
    parser.add_argument(
        "--spatial_branch", type=bool, default=True, help="whether use spatial branch"
    )
    parser.add_argument(
        "--spectral_branch", type=bool, default=True, help="whether use spectral branch"
    )
    parser.add_argument(
        "--spatial_interaction",
        type=bool,
        default=True,
        help="whether use spatial interaction",
    )
    parser.add_argument(
        "--spectral_interaction",
        type=bool,
        default=True,
        help="whether use spectral interaction",
    )
    parser.add_argument(
        "--stage_interaction",
        type=bool,
        default=True,
        help="whether use stage interaction",
    )
    parser.add_argument(
        "--block_interaction",
        type=bool,
        default=True,
        help="whether use block interaction",
    )
    parser.add_argument(
        "--in_dim", type=int, default=28, help="model's input dimension"
    )
    parser.add_argument(
        "--out_dim", type=int, default=28, help="model's output dimension"
    )
    ...
    parser.add_argument(
        "--DW_Expand", type=int, default=1, help="expand of depth-wise convolution"
    )
    parser.add_argument(
        "--ffn_name",
        type=str,
        default="Gated_Dconv_FeedForward",
        help="which feedforward function to use",
    )
    parser.add_argument(
        "--FFN_Expand", type=int, default=2.66, help="expand of FeedForward Network"
    )
    ...
    parser.add_argument(
        ...
        type=str,
        default="BiasFree",
        help="which LayerNorm type to use",
    )
    parser.add_argument(
        "--act_fn_name",
        type=str,
    ...
    parser.add_argument(
        ...
        type=int,
        default=1,
        help="whether stage body share parameters",
    )
...
def DWPWConv(
    in_dim, out_dim, kernel_size, stride, padding, bias=False, act_fn_name="gelu"
):
...
class BlockInteraction(nn.Module):
    def __init__(self, in_channel, out_channel, act_fn_name="gelu", bias=False):
        ...
        self.conv = nn.Sequential(
            ...
            ACT_FN[act_fn_name],
            nn.Conv2d(
                out_channel, out_channel, kernel_size=3, stride=1, padding=1, bias=bias
            ),
        )
...
class StageInteraction(nn.Module):
    def __init__(self, dim, act_fn_name="lrelu", bias=False):
        ...
        self.appendix = DWConv(dim, 3, 1, 1, bias=bias)
    ...
    def forward(self, inp, pre_enc, pre_dec):
        ...
        appendix = torch.sigmoid(self.appendix(skip))
        gamma = self.gamma(skip)

        out = appendix * inp + gamma
...
class DW_Inception(nn.Module):
    def __init__(self, in_dim, out_dim, bias=False):
        ...
        self.branch0 = BasicConv2d(
            in_dim, out_dim // 4, kernel_size=1, stride=1, bias=bias
        )
...
class SpatialBranch(nn.Module):
    def __init__(self, dim, DW_Expand=2, bias=False, LayerNorm_type="WithBias"):
        super().__init__()
        self.norm = LayerNorm(dim, LayerNorm_type=LayerNorm_type)
        self.inception = DW_Inception(in_dim=dim, out_dim=dim * DW_Expand, bias=bias)
...
class MixS2_Block(nn.Module):
    def __init__(
        ...
        if opt.spatial_interaction:
            self.spatial_interaction = nn.Conv2d(
                dw_channel, 1, kernel_size=1, bias=opt.bias
            )
    ...
    def forward(self, x):
        ...
        if self.opt.spectral_interaction:
            ...
            spectral_interaction = torch.sigmoid(spectral_interaction).tile(
                (1, 1, h, w)
            )
...
class MixS2_Transformer(nn.Module):
    def __init__(self, opt):
        ...
        self.embedding = nn.Conv2d(
            opt.in_dim, opt.dim, kernel_size=1, stride=1, padding=0, bias=opt.bias
        )
        ...
        if opt.stage_interaction:
            self.stage_interactions = nn.ModuleList(
                [
                    StageInteraction(
                        dim=opt.dim * 2**0, act_fn_name=opt.act_fn_name, bias=opt.bias
                    ),
                    StageInteraction(
                        dim=opt.dim * 2**1, act_fn_name=opt.act_fn_name, bias=opt.bias
                    ),
                    StageInteraction(
                        dim=opt.dim * 2**2, act_fn_name=opt.act_fn_name, bias=opt.bias
                    ),
                    StageInteraction(
                        dim=opt.dim * 2**1, act_fn_name=opt.act_fn_name, bias=opt.bias
                    ),
        ...
        self.mapping = nn.Conv2d(
            opt.dim, opt.out_dim, kernel_size=1, stride=1, padding=0, bias=opt.bias
        )
    ...
    def forward(self, x, enc_outputs=None, bottleneck_out=None, dec_outputs=None):
        ...
        if (
            self.opt.stage_interaction
            and (enc_outputs is not None)
            and (dec_outputs is not None)
        ):
        ...
        if (
            ...
            dec_res2 = self.stage_interactions[3](
                dec_res2, enc_outputs[1], dec_outputs[1]
            )
        ...
        if (
            ...
            dec_res1 = self.stage_interactions[4](
                dec_res1, enc_outputs[0], dec_outputs[0]
            )
...
class DLGD(nn.Module):
    def __init__(self, opt):
        super().__init__()
        self.func_A = opt.func_A
        self.func_A_inv = opt.func_A_inv
        # self.DL = nn.Sequential(
        #     PWDWPWConv(
        #         opt.y_dim,
        #         opt.y_dim,
        #         opt.bias,
        #         act_fn_name=opt.act_fn_name,
        #     ),
        #     PWDWPWConv(
        #         opt.y_dim,
        #         opt.y_dim,
    ...
    def forward(self, y, xk_1, appendix):
        """
        y    : (B, L, 256, 310)
        xk_1 : (B, C, 256, 310)
        appendix  : (B, 28, 256, 310)
        """
        # DL_y = self.DL(torch.cat([y.unsqueeze(1)], axis=1))
        # r = DL_y + appendix
        phi_x = self.func_A(xk_1, appendix)  # (B, 256, 310)
        yd = phi_x - y
        appendixt = self.func_A_inv(yd, appendix)
        r = self.r
        vk = xk_1 - r * appendixt
...
class DUF_MixS2_intf(nn.Module):
    def __init__(self, opt, in_channels, out_channels, func_A, func_A_inv):
        super().__init__()
        self.opt = opt
        self.func_A = func_A
        self.func_A_inv = func_A_inv
        opt.func_A = func_A
        opt.func_A_inv = func_A_inv
        opt.y_dim = in_channels
        ...
        self.body_GD = (
            nn.ModuleList([DLGD(opt) for _ in range(opt.stage - 2)])
            if not opt.body_share_params
            else DLGD(opt)
        )
        self.body_PM = (
            nn.ModuleList([MixS2_Transformer(opt) for _ in range(opt.stage - 2)])
            if not opt.body_share_params
            else MixS2_Transformer(opt)
        )
        self.tail_GD = DLGD(opt)
        self.tail_PM = MixS2_Transformer(opt)

    def forward(self, y, appendix):
        # print(f"当前分配的显存大小: {torch.cuda.memory_allocated() / 1024 / 1024} MB")

        b, l, h_inp, w_inp = y.shape
        # t0 = time.perf_counter() * 1000
        # log_dict = defaultdict(list)
        x0 = self.func_A_inv(y, appendix)
        # t1 = time.perf_counter() * 1000
        v_pre = self.head_GD(y, x0, appendix)
        ...
        for i in range(self.opt.stage - 2):
            # print(
            #     f"当前分配的显存大小: {torch.cuda.memory_allocated() / 1024 / 1024} MB"
            # )
            v_pre = (
                self.body_GD[i](y, x_pre, appendix)
                if not self.opt.body_share_params
                else self.body_GD(y, x_pre, appendix)
            )
            # v_pre = shift_back_3d(v_pre)
            # log_dict[f"stage{i+1}_v"] = v_pre
            x_pre, enc_outputs, bottolenect_out, dec_outputs = (
                self.body_PM[i](v_pre, enc_outputs, bottolenect_out, dec_outputs)
                if not self.opt.body_share_params
                else self.body_PM(v_pre, enc_outputs, bottolenect_out, dec_outputs)
            )
            # x_pre = shift_3d(x_pre)
            # log_dict[f"stage{i+1}_x"] = x_pre

        v_pre = self.tail_GD(y, x_pre, appendix)
        # v_pre = shift_back_3d(v_pre)
        # log_dict[f"stage{self.opt.stage-1}_v"] = v_pre
        out, enc_outputs, bottolenect_out, dec_outputs = self.tail_PM(
            v_pre, enc_outputs, bottolenect_out, dec_outputs
        )
