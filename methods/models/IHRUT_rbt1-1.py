...
from .sert import ChannelAttention
# 简化版
...
class TransBlock(nn.Module):
    ...
    def forward(self, x):
        ...
        x = shortcut + x  # self.sc_attn(x) ##abl5
...
class IHRTrans(nn.Module):
    ...
    def forward(self, x, last_feats):
        ...
        if last_feats is None:
            lf1 = x1
        res1 = self.cross_stage[0](torch.cat([x1, lf1], dim=1))  # abl4
        # res1 = x1
...
class DataBlock(nn.Module):
    def __init__(self, y_channels, out_channels, stage=0):
        ...
        if stage != 0:
            ...
            # self.sc_attn = MAUN_intf.CA(out_channels)
            self.sc_attn = ChannelAttention(num_feat=out_channels)
    ...
    def forward(self, x_i, y, func_A, func_A_inv, appendix, alpha, k_last):
        ...
        if self.stage == 0:
            ...
            # mom = kai + self.sc_attn(x_i - k_last)
            # mom = self.sc_attn(kai)  # abl3
            # mom = kai  # abl2
        r_i = x_i + mom  # 1
        return r_i
...
class IHRUT(nn.Module):
    def __init__(
        ...
        # self.para_estimator = MAUN_intf.CA(n_feats=in_channels)
        # self.para_estimator = ChannelAttention(num_feat=in_channels)
    ...
    def forward(self, y, appendix):
        ...
        # alphas = self.para_estimator(y)[:,:self.num_stages]
        k_cur = x
        cross_last = None
        ...
        for i in range(self.num_stages):
            ...
            alpha = alphas[:, i, :, :]
            # alpha = torch.full((1,), 0.5, device=y.device)  # abl1
            x = self.data_block[j](
