...
class Attention(nn.Module):
    ...
    def forward(self, x):
        ...
        x = rearrange(
            x, "b c (h b0) (w b1) -> (b h w) c b0 b1", b0=w_size[0], b1=w_size[1]
        )
        ...
        if (self.depth) % 2:
            out = torch.roll(
                out, shifts=(self.shift_size, self.shift_size), dims=(2, 3)
            )
        return out
...
class TransformerBlock(nn.Module):
    def __init__(
        self, dim, num_heads, ffn_expansion_factor, bias, LayerNorm_type, depth
    ):
...
class SAM(nn.Module):
    def __init__(self, in_c, bias=True):
        ...
        self.conv3 = nn.Conv2d(in_c, in_c, 1, bias=bias)
...
class DegraBlock(nn.Module):
    def __init__(self, in_c=28):
        ...
        self.attention = SAM(in_c=in_c)

    def forward(self, x_i, y, func_A, func_A_inv, appendix):
        # compute r_k
        yb = func_A(x_i, appendix)
        r = self.attention(x_i)
        # print(r.shape)
        r_i = x_i + r * func_A_inv((y - yb), appendix)
        return r_i


class MPRBlock(nn.Module):
    def __init__(self, in_c, n_feat=80, n_depth=3):
        ...
        self.conv_in = nn.Conv2d(in_c, n_feat, 3, 1, 1, bias=True)
        self.gdm = DegraBlock(in_c=in_c)
        self.conv_out = nn.Conv2d(n_feat, in_c, 3, 1, 1, bias=True)

    def forward(self, stage_img, y, f_encoder, f_decoder, func_A, func_A_inv, appendix):
        b, c, w, h = stage_img.shape
        # crop = stage_img.clone()
        # x_k_1 = shift_3d(crop[:, :, :256, :310])
        # compute r_k
        r_k = self.gdm(stage_img, y, func_A, func_A_inv, appendix)
        r_k = self.conv_in(r_k)  ##
        ...
        stage_img = stage_img[:, :, :h_inp, :w_inp]
        stage_img = self.conv_out(stage_img)  ##
        return stage_img, feat1, f_decoder


##########################################################################
class PADUT_intf(nn.Module):
    def __init__(
        self,
        in_channels,
        out_channels,
        ...
        super(PADUT_intf, self).__init__()
        self.func_0 = func_0
        self.func_A = func_A
        ...
        self.conv_in = nn.Conv2d(out_channels, n_feat, 3, 1, 1, bias=True)
        self.conv_out = nn.Conv2d(n_feat, out_channels, 3, 1, 1, bias=True)
        self.stage_model = nn.ModuleList(
            [
                MPRBlock(in_c=out_channels, n_feat=n_feat, n_depth=n_depth)
        ...
        self.gdm = DegraBlock(in_c=out_channels)

    # def initial(self, y, appendix):
    #     """
    #     :param y: [b,256,310]
    ...
    def forward(self, y, appendix):
        ...
        x_0 = self.func_A_inv(y, appendix)
        r_0 = self.gdm(x_0, y, self.func_A, self.func_A_inv, appendix)
        ...
        for i in range(self.nums_stages):
            stage_img, feat1, f_decoder = self.stage_model[i](
                stage_img, y, feat1, f_decoder, self.func_A, self.func_A_inv, appendix
            )
            output_.append(stage_img)

        return output_[-1][:, :, :h_inp, :w_inp]
